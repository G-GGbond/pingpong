...
bool EspMqtt::Connect(const std::string broker_address, int broker_port, const std::string client_id, const std::string username, const std::string password) {
    ...
    mqtt_config.broker.verification.certificate = (const char *)my_ca_cert_pem_start;
    mqtt_config.broker.verification.certificate_len = my_ca_cert_pem_end - my_ca_cert_pem_start;
...
void EspMqtt::MqttEventCallback(esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    bool Is_publish_success=false;
    switch (event_id) {
    case MQTT_EVENT_CONNECTED:
        connected_ = true;
        xEventGroupSetBits(event_group_handle_, MQTT_CONNECTED_EVENT);
        Is_publish_success = Subscribe("testtopic/hello");
        if (Is_publish_success) {
            ESP_LOGE(TAG, "Failed to subscribe to test topic");
        }
        break;
